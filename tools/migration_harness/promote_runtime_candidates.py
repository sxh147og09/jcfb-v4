#!/usr/bin/env python3
"""Promote the reviewed V4 design migrations into local runtime candidates.

This is a reproducible, source-preserving promotion step. It never edits
database/migrations/v4; it copies the reviewed design files into a separate
...
SOURCE_DIR = ROOT / "database" / "migrations" / "v4"
TARGET_DIR = ROOT / "database" / "migrations" / "v4_runtime_candidate"
MANIFEST_MD = TARGET_DIR / "0000_runtime_candidate_manifest.md"
MANIFEST_JSON = TARGET_DIR / "0000_runtime_candidate_manifest.json"

MIGRATIONS = [
    (1, "0001_prerequisites.sql", "v4-prerequisites"),
    (2, "0002_registries_core.sql", "v4-registries-core"),
    (3, "0003_market_context.sql", "v4-market-context"),
    (4, "0004_frozen_runtime.sql", "v4-frozen-runtime"),
    (5, "0005_evaluation.sql", "v4-evaluation"),
    (6, "0006_governance_audit.sql", "v4-governance-audit"),
    (7, "0007_security_rls.sql", "v4-security-rls"),
    (8, "0008_views_projections.sql", "v4-views-projections"),
    (9, "0009_seed_and_smoke.sql", "v4-seed-smoke"),
]
...
RUNTIME_SECURITY_FUNCTIONS_SQL = r"""
...
def rewrite_comments(body: str) -> str:
    replacements = {
        "-- Candidate PostgreSQL/Supabase DDL only. No connection directive, psql meta\n-- command, or automatic execution path is present. Apply only after the V4\n-- preflight, canonical hash, human approval, and acceptance gates pass.": "-- Candidate PostgreSQL DDL only. It has no connection directive, psql meta command,\n-- or automatic execution path. Apply only after the V4 preflight, canonical hash,\n-- human approval, and acceptance gates pass.",
        "-- Candidate DDL only; do not connect, apply, or seed runtime data.": "-- This candidate creates schema only; it seeds no business/runtime rows.",
        "-- Candidate source/evidence DDL only; do not execute or load market data.": "-- This candidate creates source/evidence schema only; it loads no market data.",
        "-- Candidate Frozen Input/runtime DDL only; no model or runtime execution.": "-- This candidate creates Frozen Input/runtime schema only; it performs no model execution.",
        "-- Candidate evaluation/governance DDL only; no results, reviews, or Tier A rows are seeded.": "-- This candidate creates evaluation schema only; it seeds no results, reviews, or Tier A rows.",
        "-- Candidate governance/audit DDL only; no release, incident, or audit row is seeded.": "-- This candidate creates governance/audit schema only; it seeds no release or incident business rows.",
        "-- Candidate security/trigger DDL only; no role grants or policies are applied.": "-- This candidate installs the disposable security, trigger, RLS, and role boundary.",
        "-- Candidate public projection/view DDL only; no Production output is published.": "-- This candidate creates the public projection boundary; it publishes no Production output.",
        "-- Candidate static registry/smoke DDL only; no matches, predictions, or\n-- result rows are inserted. This file is not an execution script.": "-- This candidate records only DRAFT/PENDING acceptance metadata; it inserts no match, prediction,\n-- or result rows. It is executable only on the disposable target named by the manifest.",
        "-- All complex validators are fail-closed design stubs until their reviewed\n-- implementation and test evidence are approved. None may silently repair a\n-- mismatch or manufacture a missing hash/time/source.": "-- All runtime validators below are reviewed fail-closed gates. None repairs a\n-- mismatch or manufactures a missing hash/time/source.",
        "-- This fail-closed stub prevents accidental partial adoption.": "-- This fail-closed audit contract prevents accidental partial adoption.",
        "-- Candidate lineage/time trigger attachments. The fail-closed stubs above are\n-- intentionally not usable as a migration implementation.": "-- Candidate lineage/time trigger attachments use the active fail-closed gates above.",
        "-- TODO_DECISION: verify deployed PostgreSQL security_invoker support before\n-- applying this view shape. The disposable candidate pins PostgreSQL 16+;\n-- production support remains a separate review gate.": "-- Disposable-safe decision: the runtime compose pins PostgreSQL 16+, which supports\n-- security_invoker. Production support/version confirmation remains PRODUCTION_REVIEW_REQUIRED.",
        "-- TODO_DECISION: confirm service_role exists and remains server-side. The\n-- following is a candidate least-privilege backend grant shape; it is not a\n-- browser grant and must be replaced by a custom backend role if approved.": "-- Disposable-safe decision: service_role is a no-login local server-side test role.\n-- Production ownership, mapping, and least-privilege grant review remain PRODUCTION_REVIEW_REQUIRED.",
        "-- TODO_DECISION: approve target implementation and canonical byte profile": "-- Disposable-safe decision: keep this registry row DRAFT/PENDING until the approved canonicalizer\n-- and production target byte profile are reviewed; this candidate never promotes it automatically",
    }
    for old, new in replacements.items():
        body = body.replace(old, new)
    body = body.replace("TODO_DECISION", "PRODUCTION_REVIEW_REQUIRED")
    body = body.replace("DESIGN ONLY - DO NOT APPLY", "RUNTIME VALIDATION CANDIDATE")
    body = body.replace("This file is not an execution script.", "This candidate is executable only on the disposable target named by the manifest.")
    body = body.replace("design-only manifest entry", "disposable-candidate manifest entry")
    body = body.replace("Design-only manifest entry", "disposable-candidate manifest entry")
    body = body.replace("future implementation", "approved production implementation")
    body = body.replace("future smoke harness", "runtime smoke harness")
    body = body.replace("Future smoke harness", "Runtime smoke harness")
    body = body.replace("this design file", "this candidate file")
    body = body.replace(
        "no-future-leakage rules are not accepted until the reviewed functions exist.",
    ...
    body = body.replace("This design never invents a canonical hash.", "This candidate never invents a canonical migration hash.")
    return body


def make_body(source_name: str, sequence: int, source_commit: str) -> str:
    source_text = (SOURCE_DIR / source_name).read_text(encoding="utf-8")
    body = source_text.split("\n", 1)[1]
    body = rewrite_comments(body)
    body = body.replace("CREATE EXTENSION IF NOT EXISTS pgcrypto;", "CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA public;", 1)
    ...
    if sequence == 1:
        ...
        body = re.sub(
            r"-- PRODUCTION_REVIEW_REQUIRED: confirm these namespaces.*?-- timezone remain explicit business fields; Asia/Shanghai is not inferred\.",
    ...
    if sequence == 7:
        start = body.index("CREATE FUNCTION governance.reject_append_only_mutation()")
        end = body.index("-- Registry metadata is the only controlled UPDATE path.")
        body = body[:start] + RUNTIME_SECURITY_FUNCTIONS_SQL + "\n\n" + body[end:]
    if sequence == 9:
        body = body.replace(
            "'PRODUCTION_REVIEW_REQUIRED: approve target implementation and canonical byte profile'",
            "'Disposable candidate keeps the registry DRAFT/PENDING; production canonicalizer and byte profile remain PRODUCTION_REVIEW_REQUIRED'",
        )
    header = f"""-- JCFB V4 RUNTIME VALIDATION CANDIDATE
-- RUNTIME VALIDATION CANDIDATE
-- DISPOSABLE/STAGING ONLY
-- NOT APPROVED FOR PRODUCTION
-- candidate_identity: v4-runtime-candidate@20260901.{sequence:03d}
-- source_design_file: database/migrations/v4/{source_name}
-- source_design_commit: {source_commit}
...
def main() -> int:
    if not SOURCE_DIR.is_dir():
        print(f"source directory missing: {SOURCE_DIR}", file=sys.stderr)
        return 2
    TARGET_DIR.mkdir(parents=True, exist_ok=True)
    expected_names = [name for _, name, _ in MIGRATIONS]
    existing = sorted(p.name for p in TARGET_DIR.glob("*.sql"))
    unexpected = sorted(set(existing) - set(expected_names))
    if unexpected:
        print(f"unexpected candidate files already present: {unexpected}", file=sys.stderr)
        return 2

    current_commit = run_git("rev-parse", "HEAD")
    source_manifest = SOURCE_DIR / "0000_manifest.md"
    source_manifest_commit = run_git("log", "-1", "--format=%H", "--", str(source_manifest.relative_to(ROOT)))
    candidates: list[dict[str, object]] = []
    for sequence, source_name, name in MIGRATIONS:
        source_path = SOURCE_DIR / source_name
        if not source_path.is_file():
            print(f"missing source migration: {source_path}", file=sys.stderr)
            return 2
        source_commit = run_git("log", "-1", "--format=%H", "--", str(source_path.relative_to(ROOT)))
        candidate_text = make_body(source_name, sequence, source_commit)
        candidate_path = TARGET_DIR / source_name
        candidate_path.write_text(candidate_text.rstrip() + "\n", encoding="utf-8", newline="\n")
        candidates.append(
            {
                "sequence": sequence,
                "name": name,
                "source_design_file": str(source_path.relative_to(ROOT)).replace("\\", "/"),
                "source_design_commit": source_commit,
                "candidate_file": str(candidate_path.relative_to(ROOT)).replace("\\", "/"),
                "candidate_identity": f"v4-runtime-candidate@20260901.{sequence:03d}",
                "migration_id": f"migration@20260901.{sequence:03d}",
                "depends_on": [] if sequence == 1 else [f"migration@20260901.{sequence - 1:03d}"],
                "canonical_migration_hash": "PENDING_CANONICAL_HASH",
                "content_sha256_noncanonical": sha256_bytes(candidate_text.encode("utf-8")),
                "status": "RUNTIME_VALIDATION_CANDIDATE",
                "production_approval": "NOT_APPROVED_FOR_PRODUCTION",
            }
        )

    manifest = {
        "$id": "v4-runtime-candidate-manifest@1.0.0",
        "title": "JCFB V4 disposable runtime validation candidate manifest",
        "task": "PRE-BATCH-04 REMEDIATION",
        "candidate_directory": str(TARGET_DIR.relative_to(ROOT)).replace("\\", "/"),
        "source_manifest": str(source_manifest.relative_to(ROOT)).replace("\\", "/"),
        "source_manifest_commit": source_manifest_commit,
        "promotion_commit": current_commit,
        "candidate_count": len(candidates),
        "environment_allowlist": ["DISPOSABLE_LOCAL", "STAGING"],
        "production_apply": "HARD_BLOCK",
        "canonical_hash_status": "PENDING_CANONICAL_HASH",
        "candidates": candidates,
        "dependency_graph": [
            {"migration_id": c["migration_id"], "depends_on": c["depends_on"]}
            for c in candidates
        ],
        "disposable_safe_decisions": [
            "Fresh local PostgreSQL 16 container only; no production connection string is accepted.",
        ...
        "runtime_execution_policy": {
            "allowed_target_identity": "DISPOSABLE_LOCAL or explicitly approved STAGING",
            "production_apply": "FORBIDDEN",
            "v333_mutation": "FORBIDDEN",
            "business_seed_data": "NONE",
            "next_gate": "Pre-BATCH-04 Runtime Validation Gate after disposable DB bootstrap",
        },
    }
    ...
    for c in candidates:
        deps = ", ".join(c["depends_on"]) if c["depends_on"] else "none"
        lines.append(
            f"| {c['sequence']:04d} | {c['candidate_file']} | {c['source_design_file']} @ {c['source_design_commit'][:12]} | {deps} | {c['content_sha256_noncanonical']} |"
        )
    lines.extend(
        [
            "",
    ...
    print(f"generated {len(candidates)} runtime candidates in {TARGET_DIR}")
